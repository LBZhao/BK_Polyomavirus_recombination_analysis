...
nonhomo=open('./Nonhomology.csv','w+')
yeshomo=open('./Yeshomo.csv','w+')
...
for record in SeqIO.parse(sys.argv[1], "fasta"):
    ...
    for j in range(n-1):
        ...
        if microhomologyfinder(jointfinder(output1,output2,output3))) == 0:
            nonhomocatch.append((leftsubjectend,rightsubjectstart))
        elif microhomologyfinder(jointfinder(output1,output2,output3))) > 0:
