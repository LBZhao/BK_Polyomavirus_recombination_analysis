...
for record in SeqIO.parse(sys.argv[1], "fasta"):
    ...
    if updated:
        ...
        if leftdirection=='1/1':
            ...
            output1=' '*(leftstart-1) + leftsubject+BKloopr[5404-leftsubjectend:5414-leftsubjectend]+' '*(241-leftend)
        output2=record.seq
        if rightdirection=='1/1':
            output3=' '*(rightstart-11) +BKloop[rightsubjectstart-11:rightsubjectstart-1]+ rightsubject+' '*(241-rightend)
        else:
            output3=' '*(rightstart-11) +BKloopr[5393-rightsubjectstart:5403-rightsubjectstart]+ rightsubject+' '*(241-rightend)

        #print section 1
        print('%8s' % leftsubjectstart + '  ' + output1[:100] + ' '*3 + leftname)
        print('%8s' % '1' + '  ' + output2[:100] + ' ' * 3 + 'Read')
        print('%8s' % rightsubjectstart + '  ' + output3[:100]+ ' '*3 + rightname)
        print('\n')

        #print section 2
        if leftdirection=='1/1':
            print('%8s' % (leftsubjectstart+100) + '  '+output1[100:200]+' '*3+leftname)
        else:
            print('%8s' % (leftsubjectstart-100) + '  '+output1[100:200]+' '*3+leftname)
        print('%8s' % '101'+'  '+output2[100:200]+' '*3+'Read')
        if rightdirection=='1/1':
            print('%8s' % (rightsubjectstart+ 100) +'  '+output3[100:200]+' '*3+rightname)
        else:
            print('%8s' % (rightsubjectstart- 100) +'  '+output3[100:200]+' '*3+rightname)
        print('\n')
        ...
        #print section 3
        if leftdirection=='1/1':
            print('%8s' % (leftsubjectstart+200)+'  '+output1[200:]+ '  ' + '%5s' % leftdirection + '  '+'3\'end-'+ '%-8s' % leftsubjectend + ' '*3+leftname)
        else:
            print('%8s' % (leftsubjectstart-200)+'  '+output1[200:]+ '  ' + '%5s' % leftdirection + '  '+ '3\'end-'+'%-8s' % leftsubjectend + ' '*3+leftname)
        print('%8s' % '201'+'  '+output2[200:]+ '  ' + '  N/A'+ '  '+ '3\'end-'+'%-8s' % '251' + ' '*3+ 'Read')
        if rightdirection=='1/1':
            print('%8s' % (rightsubjectstart+200)+'  '+output3[200:]+ '  ' + '%5s' % rightdirection + '  '+ '3\'end-'+'%-8s' % rightsubjectend + ' '*3+rightname)
        else:
            print('%8s' % (rightsubjectstart-200)+'  '+output3[200:]+ '  ' + '%5s' % rightdirection + '  '+ '3\'end-'+'%-8s' % rightsubjectend + ' '*3+rightname)
        print('\n')
        print('\n')
